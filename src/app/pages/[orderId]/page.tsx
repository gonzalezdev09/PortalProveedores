...
import { OrderDetail, OrderStatus, OrderType, InvoicePaymentStatus, PartQuality, RepairSubStatus } from '../../types'; // Ajusta la ruta si types.ts está en otro lugar
...
const fetchOrderDetail = async (orderId: string): Promise<OrderDetail | null> => {
  ...
  const mockDetails: { [key: string]: OrderDetail } = {
    ...
    '3': { // Orden en FACTURACION (sin factura cargada inicialmente)
      ...
      adjuster: 'Pedro Gómez', analyst: 'Ana Torres', insuredName: 'Carlos López', clientIdNumber: '001-0001112-3', contactPhone: '809-555-4444', policyNumber: 'POL-5555',
      vehicleMake: 'Hyundai', vehicleModel: 'Tucson', vehicleYear: 2018, vehicleVin: 'KLM123ABC456', vehiclePlate: 'A555666', vehicleColor: 'Gris',
      history: [ { timestamp: '2024-04-15T10:00:00Z', description: 'Reparación finalizada, pendiente de facturación.', status: OrderStatus.BILLING, user: 'Taller Jefe' }, ],
      invoiceUrl: null, invoicePaymentStatus: null, invoiceNcf: null, invoiceDueDate: null, invoiceAmount: null,
    },
     '4': { // Orden PAGADA (con factura y estado de pago)
      ...
      adjuster: 'Luisa Méndez', analyst: 'Carlos Ruiz', insuredName: 'Laura Fernández', clientIdNumber: '001-3334445-6', contactPhone: '829-555-7777', policyNumber: 'POL-8888',
      vehicleMake: 'Mazda', vehicleModel: 'CX-5', vehicleYear: 2022, vehicleVin: 'EFG456HIJ789', vehiclePlate: 'A999888', vehicleColor: 'Negro',
      history: [ { timestamp: '2024-03-15T10:00:00Z', description: 'Inspección realizada y facturada.' }, { timestamp: '2024-03-20T14:00:00Z', description: 'Pago recibido.', status: OrderStatus.PAID, user: 'Administración' }, ],
      invoiceUrl: '/uploads/mock-invoice-004.pdf', invoicePaymentStatus: InvoicePaymentStatus.PAID,
      invoiceNcf: 'B0100001234', invoiceDueDate: '2024-03-30', invoiceAmount: 5500.00,
    },
...
export default function OrderDetailPage() {
  ...
  // ** Nuevos estados para formulario de factura **
  const [invoiceNcf, setInvoiceNcf] = useState<string>('');
  const [invoiceDueDate, setInvoiceDueDate] = useState<string>('');
  const [invoiceAmount, setInvoiceAmount] = useState<string>(''); // Usar string para input

  useEffect(() => { /* ... (Resetear nuevos estados en la carga) ... */
    if (orderId) {
  ...
  // ** Modificar handleUploadInvoice para incluir nuevos campos **
  const handleUploadInvoice = async () => {
      // Validaciones
      if (!selectedInvoiceFile) { alert('Por favor, seleccione un archivo de factura.'); return; }
      if (!invoiceNcf.trim()) { alert('Por favor, ingrese el NCF.'); return; }
      if (!invoiceDueDate) { alert('Por favor, seleccione la fecha de vencimiento.'); return; }
      const amount = parseFloat(invoiceAmount);
      if (isNaN(amount) || amount <= 0) { alert('Por favor, ingrese un monto facturado válido.'); return; }

      setIsUploadingInvoice(true);
      console.log('Simulando subida de factura con datos:', {
      ...
      setOrderDetail(prev => prev ? {
          ...prev,
          invoiceUrl: `/uploads/mock-invoice-${orderId}.pdf`, // URL simulada
          invoicePaymentStatus: InvoicePaymentStatus.PENDING_PAYMENT, // Estado inicial tras subir
          invoiceNcf: invoiceNcf,
          invoiceDueDate: invoiceDueDate,
          invoiceAmount: amount,
      } : null);

      alert(`Factura "${selectedInvoiceFile.name}" y datos asociados subidos (simulado). Estado de pago: Pendiente.`);
      ...
      // Aquí iría la lógica real de subida y actualización en backend
  };
                    ...
                    <div>
                        ...
                        <div className={styles.billingForm}>
                            ...
                            <div className={styles.billingFormGroup}>
                                <label htmlFor="invoiceNcf" className={styles.billingLabel}>NCF:</label>
                                <input
                                    type="text"
                                    id="invoiceNcf"
                                    value={invoiceNcf}
                                    onChange={(e) => setInvoiceNcf(e.target.value)}
                            ...
                            {/* Fecha Vencimiento */}
                            <div className={styles.billingFormGroup}>
                                <label htmlFor="invoiceDueDate" className={styles.billingLabel}>Fecha Vencimiento:</label>
                                <input
                                    type="date"
                                    id="invoiceDueDate"
                             ...
                             {/* Monto Facturado */}
                             <div className={styles.billingFormGroup}>
                                <label htmlFor="invoiceAmount" className={styles.billingLabel}>Monto Facturado:</label>
                                <input
                                    type="number"
                                    id="invoiceAmount"
                                    value={invoiceAmount}
                            ...
                            <div className={styles.billingFileInputGroup}>
                                <label htmlFor="invoiceUpload" className={styles.billingLabel}>Archivo Factura:</label>
                                <div> {/* Contenedor para alinear botón y nombre */}
                                    <label htmlFor="invoiceUpload" className={styles.fileInputLabel}>
                                        Seleccionar Archivo
                                    </label>
                                    <input
                                        type="file"
                        ...
                        <div className={styles.buttonContainer} style={{marginTop: '20px'}}>
                            <button
                                ...
                                className={`${styles.actionButtonBase} ${styles.primaryButton}`}
                                disabled={!selectedInvoiceFile || !invoiceNcf || !invoiceDueDate || !invoiceAmount || isUploadingInvoice}
                            >
                                {isUploadingInvoice ? 'Subiendo...' : 'Subir Factura y Datos'}
                            </button>
                        </div>
                    </div>
                ) : (
                    ...
                    <div>
                         ...
                         <div className={styles.invoiceInfo}>
                             ...
                             <div className={styles.invoiceDataItem}>
                                 <span className={styles.invoiceDataLabel}>NCF:</span>
                                 <span className={styles.invoiceDataValue}>{orderDetail.invoiceNcf || 'N/D'}</span>
                             </div>
                              {/* Monto */}
                             <div className={styles.invoiceDataItem}>
                                 <span className={styles.invoiceDataLabel}>Monto:</span>
                                 <span className={styles.invoiceDataValue}>
                                     {orderDetail.invoiceAmount ? `$${orderDetail.invoiceAmount.toFixed(2)}` : 'N/D'}
                                 </span>
                             </div>
                             {/* Fecha Vencimiento */}
                             <div className={styles.invoiceDataItem}>
                                 <span className={styles.invoiceDataLabel}>Vencimiento:</span>
                                 <span className={styles.invoiceDataValue}>
                                     {orderDetail.invoiceDueDate ? new Date(orderDetail.invoiceDueDate+'T00:00:00').toLocaleDateString('es-DO') : 'N/D'}
                                 </span>
                             </div>
                             {/* Enlace Factura */}
                             <div className={styles.invoiceDataItem}>
                                <span className={styles.invoiceDataLabel}>Factura:</span>
                                <span className={`${styles.invoiceDataValue} ${styles.invoiceLink}`}>
                                    <a href={orderDetail.invoiceUrl} target="_blank" rel="noopener noreferrer">Ver Archivo</a>
                                </span>
                             </div>
                             {/* Estado Pago */}
                             <div className={styles.invoiceDataItem}>
                                 <span className={styles.invoiceDataLabel}>Estado Pago:</span>
                                 <span className={`${styles.paymentStatusBadge} ${getPaymentStatusBadgeClass(orderDetail.invoicePaymentStatus)}`}>
                                    {orderDetail.invoicePaymentStatus || 'N/D'}
                                 </span>
                             </div>
                         </div>
                         {/* Botón simulado para marcar como pagada */}
                         {orderDetail.invoicePaymentStatus === InvoicePaymentStatus.PENDING_PAYMENT && (
                             <div className={styles.buttonContainer} style={{marginTop: '20px'}}>
                                  <button
                                      onClick={handleMarkAsPaid}
                                      className={`${styles.actionButtonBase} ${styles.secondaryButton}`}
                                      title="Esta acción normalmente la realiza administración"
                                  >
                                      Marcar como Pagada (Sim)
                                  </button>
                             </div>
                         )}
                    </div>
